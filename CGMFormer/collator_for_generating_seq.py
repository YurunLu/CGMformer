...
class DataCollatorForGlucoseClassification(DataCollatorForTokenClassification):
    ...
    def _prepare_batch(self, features):
        ...
        additional_columns = ['id', 'types', 'age', 'bmi', 'hba1c', 'homa-b', 'homa-is', 'index', 'Fast_s', 'Fast_e', 'Dawn_s', 'Dawn_e', 'Breakfast_s', 'Breakfast_e', 'Lunch_s', 'Lunch_e', 'Dinner_s', 'Dinner_e']
        # 9.5 Update Logic
        ...
        # Extract additional columns and add them to the batch
        additional_data = {column: [feature[column] for feature in features] for column in additional_columns}

        input_ids = torch.tensor([feature['input_ids'] for feature in features], dtype=torch.long)
        attention_masks = torch.ones_like(input_ids)
        batch = {'input_ids': input_ids, 'attention_mask': attention_masks, 'additional_data':additional_data}
        return batch
...
class DataCollatorForSeqGeneration(DataCollatorForGlucoseClassification):
